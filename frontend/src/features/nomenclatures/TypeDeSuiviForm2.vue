<template>
  ...
  <form class="type-suivi-form" @submit.prevent="submitForm">
...
const { can } = usePermissions("typedesuivi");
...
const submitForm = () => {
  ...
  if (props.mode === 'add') delete payload.id_type_suivi;
...
.type-suivi-form :deep(.v-input--density-compact .v-field__input) { min-height: 38px; padding-top: 6px; padding-bottom: 6px; }
.type-suivi-form :deep(.v-label.v-field-label) { font-size: 0.82rem; }
.type-suivi-form :deep(.v-input) { font-size: 0.88rem; }
.type-suivi-form :deep(.v-field__input),
.type-suivi-form :deep(.v-select__selection-text) { font-size: 0.88rem; }
