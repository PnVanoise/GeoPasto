...
const crud = useCrudPage("logement", "logement", "id", { geojson: true });
  ...
  if (route.params.id) {
    try {
      const response = await auth.axiosInstance.get(
        `${config.API_BASE_URL}/api/logement/${route.params.id}/`
      );
      itemData.value = response.data ?? {};
    } catch (e) {
      console.error("Erreur lors du chargement du logement", e);
    } finally {
