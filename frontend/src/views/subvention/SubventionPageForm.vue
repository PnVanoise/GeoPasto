...
const crud = useCrudPage("subventionpnv", "subventionPNV", "id_subvention");
  ...
  if (route.params.id) {
    try {
      const response = await auth.axiosInstance.get(
        `${config.API_BASE_URL}/api/subventionPNV/${route.params.id}/`
      );
      itemData.value = response.data ?? {};
    } catch (e) {
      console.error("Erreur lors du chargement de la subvention", e);
    } finally {
